...
const fetchCompany = (req, res) => {
  const query = `SELECT * FROM company_table`;
...
const addCompany = (req, res) => {
  ...
  db.query(query, [value], (err) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error adding company", error: err });
    }
...
const deleteData = (req, res) => {
  const { id } = req.body;
  const query = `DELETE FROM company_table WHERE id = ?`;
...
module.exports = { fetchCompany, addCompany, deleteData, updateData };
