...
export default function AddHistory({ open, setAddhistory }) {
  const callTypes = [
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const historyData = {
        ...
        datetime: datetime.replace('T', ' '), // Replace 'T' with a space if necessary
        rescheduleDatetime: selectedType.includes("Reschedule") 
          ? rescheduleDatetime.replace('T', ' ') // Also format rescheduleDatetime
          : null,
        image1: uploadData?.filePaths?.[0] || null, // Check if image exists
        image2: uploadData?.filePaths?.[1] || null, // Check if image exists
      };
